...
определенные в интерфейсе Collection: ``` add(), remove, contains, size и другие```. Поэтому мы можем
использовать разные методы для одного и того же действия. Например, добавление в самое начало списка
